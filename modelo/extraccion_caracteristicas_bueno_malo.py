...
def asignar_nueva_etiqueta(emocion_original):
    if emocion_original in ["Surprised", "Happy", "Neutral"]:
        return "Buena"
    else:
        return "Mala"

# Creación nuevas etiquetas
y_nueva = np.array([asignar_nueva_etiqueta(emocion) for emocion in y])

# Codificar las nuevas etiquetas en números
label_encoder_nueva = LabelEncoder()
y_encoded_nueva = label_encoder_nueva.fit_transform(y_nueva)

np.save("labels/2labels.npy", y_encoded_nueva)
